...
export const SignIn: React.FC<SignInProps> = ({ onSignIn, onSignInWithGoogle }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    const signInError = await onSignInWithGoogle();
    if (signInError) {
      setError(signInError);
    }
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/20 shadow-2xl">
          ...
          <div className="flex items-center my-5">
            ...
            <span className="px-4 text-sm text-indigo-300/60">or</span>
            <div className="flex-1 border-t border-white/20" />
          </div>
          ...
          <button
            ...
            disabled={loading || googleLoading}
            className="w-full py-3 px-4 bg-white hover:bg-gray-50 text-gray-800 font-semibold rounded-xl shadow-lg transition-all duration-300 flex items-center justify-center space-x-3 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
