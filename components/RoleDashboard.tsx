...
export const RoleDashboard: React.FC<RoleDashboardProps> = ({
  ...
  const QuickStatsWidget = () => {
    // Calculate real total movement from transactions
    ...
    const compliancePercent = complianceRules.length > 0
      ? Math.round((compliantCount / complianceRules.length) * 100)
      : 100;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800">{todayMovement.toLocaleString()} L</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex items-center space-x-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800">{compliancePercent}%</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="space-y-4">
        {auditLogs.slice(0, limit).map(log => (
          <div key={log.id} className="flex items-start pb-4 border-b border-slate-100 last:border-0 last:pb-0">
            <div className={`w-2 h-2 mt-2 rounded-full mr-4 ${log.action === 'LOSS' ? 'bg-red-500' : 'bg-indigo-500'
              }`}></div>
